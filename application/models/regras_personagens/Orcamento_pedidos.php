 ...
 * @author Alexandre
 */
class Orcamento_pedidos extends CI_Model{
    //put your code here
    public function resumoPedidoUP($pizzaria, $fb_id){
        ...
        if( $this->validaIgredientesUltimoPedido($pizzaria, $fb_id) != true ){
            throw new Exception("Ops, fizemos alguns ajustes para melhorar os nossos serviços e não temos como repetir o seu último pedido. Agradecemos a compreensão.");
        }
        ...
        if($cliente != null && $pedido != null){
            ...
            for($i=0; $i < count($todosItemPedido); $i++){
                if($todosItemPedido[$i]["bebida_item_pedido"] != null){
                    ...
                    if($bebidas == null){return false;}
                    $respBebida = $respBebida."\n".intval($todosItemPedido[$i]["quantidade_item_pedido"])." - ".$bebidas["descricao_bebida"];
                    ...
                    if($tamanhoPizza == null){return false;}
                    $resposta=$resposta.$tamanhoPizza["descricao_tamanho_pizza"]; 
    ...
    public function retornaOrcamentoUPCliente($pizzaria, $fb_id){
        $this->load->model("pizzaria/Bebida_model");
        $this->load->model("pizzaria/Pedido_model");
        $this->load->model("pizzaria/Cliente_model");
        $this->load->model("pizzaria/Item_pedido_model");
        ...
        if($pedido != null){            
            ...
            for($i=0; $i < count($todosItemPedido); $i++){
                if($todosItemPedido[$i]["bebida_item_pedido"] != null){
                    ...
                    $bebidas =  $this->Bebida_model->buscarBebidaPorIdECodgPizzaria($todosItemPedido[$i]["bebida_item_pedido"], $pizzaria);
                    $respBebida = $respBebida."\n".intval($todosItemPedido[$i]["quantidade_item_pedido"])." - ".$bebidas["descricao_bebida"]." - valor R$".
                            number_format($bebidas["preco_bebida"]*$todosItemPedido[$i]["quantidade_item_pedido"], 2, '.', '');
                    ...
                    if($todosItemExtras != null){
                        ...
                        for($j=0; $j < count($todosItemExtras);$j++){
                            ...
                            $resposta=$resposta."\n".$tipo["descricao_tipo_extra_pizza"]." ".$extra["descricao_extra_pizza"]." - valor R$".$extra["preco_extra_pizza"];
                        }   
                    }
                } 
            }
            ...
            $resposta .= "\nValor total do pedido: R$ ".$custoTotal;
            return $resposta;
        }else{
            return false;
        }
    ...
    public function validaIgredientesUltimoPedido($pizzaria, $fb_id){
        $this->load->model("pizzaria/Bebida_model");
        $this->load->model("pizzaria/Pedido_model");
        $this->load->model("pizzaria/Cliente_model");
        $this->load->model("pizzaria/Item_pedido_model");
