...
/**
 * Description of Resumo_orcamento_pedido
 *
 * @author 033234581
 */
class Resumo_orcamento_pedido extends CI_Controller{
    //put your code here
    public function resumoUltimoPedido(){
        $this->load->model("regras_personagens/Resumo_ou_orcamento_regras");
        try{
            ...
            $resposta = $this->Resumo_ou_orcamento_regras->resumoPedidoUP($pizzaria, $idCliente, $personagem, $nome);
    ...
    public function orcamentoUltimoPedido(){
        $this->load->model("regras_personagens/Resumo_ou_orcamento_regras");
        try{
            if(!isset($_GET["Codg_pizzaria"])){throw new Exception("Codigo da Pizzaria não existente");}
            if(!isset($_GET["Codg_personagem"])){throw new Exception("Codigo da Personagem não existente");}
            ...
            $resposta = $this->Resumo_ou_orcamento_regras->orcamentoPedidoUP($pizzaria, $idCliente, $personagem, $nome);
            $json_str = json_encode($resposta);
            echo $json_str;
        }catch (Exception $e){
            $fp = fopen("log.txt", "a");
