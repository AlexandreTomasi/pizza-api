...
<script>
    ...
    function printTextArea(impressao) {
	 ...
	 childWindow.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Impressão</title></head><body style="font: bold 14px Courier New"><div class="row">');//1rem é igual a 16px
         childWindow.document.write(tudo.replace(/\n/gi,'<br>').replace(/\s/g,'&nbsp;'));
	 childWindow.document.write('</div></body></html>');
	 childWindow.print();
        ...
        function atualiza($http, $timeout) {
            ...
            function atualizar() {
                ...
                $http.post('<?= base_url("index.php/ControladorManterPedidos/buscarPedidosConstantemente") ?>', {
                    ...
                    mensagens = data;
                    if(mensagens == "Sessão expirada"){
                        alert("Sua sessão expirou, por favor relogue novamente.");
                        location.reload();
                        return "";
                    }
                    if (quantPedidos === -1) {
            ...
            function pedidoSolicitado(){
                for (numero = 0; numero < mensagens.length; numero++) {
                    if (mensagens[numero].status_pedido === "Solicitado") {
                        alert("Existe solicitação de pedido não atendido.");
                        break;
                    }
                }
            }
