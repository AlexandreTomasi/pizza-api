            ...
            <div class="container main2" ng-app="myApp" ng-controller="ControladorProduto">
                <div class="row">
                    ...
                    <div class="sorter"><h6 style="display: inline-block;">Ordenar por: </h6>
                        ...
                        <a  ng-class="{'activetab  waves-light' : predicate == 'ativo_forma_pagamento',  'disabled': predicate != 'ativo_forma_pagamento'}"
                            ng-click="predicate = 'ativo_forma_pagamento'; reverse = !reverse" class="btn">Status
                            <i ng-show="predicate == 'ativo_forma_pagamento' && !reverse" class="material-icons right">expand_more</i>
                            <i ng-show="predicate == 'ativo_forma_pagamento' && reverse" class="material-icons right">expand_less</i>
                        </a>
                    </div>
                    ...
                    <table class="table hoverable bordered">
                        <thead>
                            <tr>
                                ...
                                <th ng-class="{'activetab waves-effect waves-light' : predicate == 'ativo_forma_pagamento'}"  ng-click="predicate = 'ativo_forma_pagamento';
                        ...
                        <tbody>
                            <tr dir-paginate="produto in produtos | filter:search:buscaSemAcento | filter:filters | orderBy:predicate:reverse:localeSensitiveComparator | itemsPerPage: resultlimit ">                              
                                ...
                                <td> {{produto.ativo_forma_pagamento}} </td> 
                                <td class="text-align-center" >
                                    <a ng-hide="{{produto.codigo_forma_pagamento == 1 }}" ng-click="ativarProduto(produto.codigo_forma_pagamento)" title="ativar forma de pagamento" class="waves-light btn customAction" ng-if="produto.ativo_forma_pagamento == 'Inativo'"><i class="material-icons centered">check</i></a>
                                    <a ng-hide="{{produto.codigo_forma_pagamento == 1 }}" ng-click="inativarProduto(produto.codigo_forma_pagamento)" title="inativar forma de pagamento" class="waves-light btn customAction" ng-if="produto.ativo_forma_pagamento == 'Ativo'"><i class="material-icons centered">close</i></a>
    ...
    app.controller('ControladorProduto', function ($scope, $http) {
       ...
       $scope.localeSensitiveComparator = function(v1, v2) {
        ...
        $scope.ativarProduto = function (codigo) {
            if (confirm("Deseja reativar essa forma de pagamento? Ao ativar a forma de pagamento, o bot apresentará essa forma de pagamento novamente.")) {
                $http.post('<?= base_url("index.php/ControladorFormaPagamento/ativarFormaDePg") ?>', {
                    'codigo_forma_pagamento': codigo
                }).success(function (data, status, headers, config) {
                    // tell the user bebida record was updated
                    Materialize.toast("Dados alterados com sucesso", 4000);
                    for (numero = 0; numero < $scope.produtos.length; numero++) {
                        if (parseInt($scope.produtos[numero].codigo_forma_pagamento) === parseInt(codigo)) {
                            $scope.produtos[numero].ativo_forma_pagamento = "Ativo";
                        }
        ...
        $scope.inativarProduto = function (codigo) {
            if (confirm("Deseja inativar essa forma de pagamento? Ao inativar a forma de pagamento, o bot não apresentará essa opção até que este esteja ativo novamente.")) {
                $http.post('<?= base_url("index.php/ControladorFormaPagamento/inativarFormaDePg") ?>', {
                    'codigo_forma_pagamento': codigo
                }).success(function (data, status, headers, config) {
                    // tell the user bebida record was updated
                    Materialize.toast("Dados alterados com sucesso", 4000);
                    for (numero = 0; numero < $scope.produtos.length; numero++) {
                        if (parseInt($scope.produtos[numero].codigo_forma_pagamento) === parseInt(codigo)) {
                            $scope.produtos[numero].ativo_forma_pagamento = "Inativo";
                        }
