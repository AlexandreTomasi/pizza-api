            ...
            */
            ...
            (min-device-width: 768px) and (max-device-width: 1024px)  {
                ...
                */
                ...
                td:nth-of-type(2):before { content: "Quantidade:"; font-weight: bold;}
            }
                ...
                <div class="row">
                    <div class="col s12">
                        ...
                        <div class="input-field col s8 nopad">
                            <input type="text" ng-model="search" class="form-control" placeholder="Pesquisar tipo extra..." />
                        </div>
                    </div>
                    <br>
                    <div class="sorter"><h6 style="display: inline-block;">Ordenar por: </h6>
                        <a  ng-class="{'activetab  waves-light' : predicate === 'descricao_tipo_extra_pizza',  'disabled': predicate !== 'descricao_tipo_extra_pizza'}"
                            ng-click="predicate = 'descricao_tipo_extra_pizza'; reverse = !reverse" class="btn">Descricao
                            <i ng-show="predicate === 'descricao_tipo_extra_pizza' && !reverse" class="material-icons right">expand_more</i>
                            <i ng-show="predicate === 'descricao_tipo_extra_pizza' && reverse" class="material-icons right">expand_less</i></a>
                    </div>
                    ...
                    <table class="table hoverable bordered">
                        <thead>
                            <tr>
                                <th ng-class="{'activetab waves-effect waves-light' : predicate === 'descricao_tipo_extra_pizza'}"  ng-click="predicate = 'descricao_tipo_extra_pizza';
                                            reverse = !reverse">Descrição
                                    <i ng-show="predicate === 'descricao_tipo_extra_pizza' && !reverse" class="material-icons right">expand_more</i>
                                    <i ng-show="predicate === 'descricao_tipo_extra_pizza' && reverse" class="material-icons right">expand_less</i>
                                </th>          
                                <th >Quantidade</th>
                        ...
                        <tbody>
                            <tr dir-paginate="produto in produtos | filter:search:buscaSemAcento | filter:filters | orderBy:predicate:reverse | itemsPerPage: resultlimit ">                              
                                <td> {{produto.descricao_tipo_extra_pizza}} </td>  
                                <td > {{produto.quantidade_tipo_extra_pizza}} </td>       
                                <td class="text-align-center" >
                                    <a ng-click="carregarProduto(produto.codigo_tipo_extra_pizza)" title="alterar tipo extra" class="waves-light btn customAction" ><i class="material-icons centered">edit</i></a>
                                    <a ng-click="excluirProduto(produto.codigo_tipo_extra_pizza)" title="excluir tipo extra" class="waves-light btn customAction"><i class="material-icons centered">delete</i></a>
                                </td>    
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <form name="produtoForm" id="modal-produto-form" class="modal" novalidate>
                        <div class="modal-content">
                            <h4 id="modal-produto-title">Inserir Tipo extra</h4>
                            <div class="row">
                                <div class="input-field col s12">
                                    <input type="text" ng-model="descricaoInput" name="descricaoInput" id="descricaoInput" required aria-required="true" ng-pattern="/^[A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ ]+([0-9,.'-()]+)?/" placeholder="Nome do tipo extra"/>
                                ...
                                <div class="input-field col s12">
                                    <input type="number" ng-model="precoInput" name="precoInput" id="precoInput" required ng-pattern="/^[0-9]/"  placeholder="Quantidade máxima desse tipo extra na pizza. ex: 1" step="1" min="0"/>
                                    <label for="precoInput">Quantidade (0 - ilimitado)</label>
                                </div>
                    ...
                    <div class="fixed-action-btn" style="bottom:45px; right:24px;">
                        <a class="waves-effect waves-light btn modal-trigger btn-floating btn-large red" href="#modal-produto-form" title="inserir tipo extra" ng-click="showCreateForm()"><i class="large material-icons">add</i></a
                    </div> <!-- END BUTTON -->
...
<script>
                            ...
                            app.controller('ControladorProduto', function ($scope, $http) {
                                // read produtos
                                $scope.produtos = <?php echo json_encode($tipoExtra) ?>;
                                ...
                                $scope.showCreateForm = function () {
                                    ...
                                    $scope.clearForm();
                                    // change modal title
                                    $('#modal-produto-title').text("Inserir Tipo extra");
                                ...
                                $scope.incluirProduto = function () {
                                    ...
                                    $http.post('<?= base_url("index.php/ControladorPizzas/incluirTipoExtra") ?>', {
                                        'descricao_tipo_extra_pizza': $scope.descricaoInput,
                                        'quantidade_tipo_extra_pizza': $scope.precoInput
                                    }
                                ...
                                $scope.carregarProduto = function (id) {
                                    // change modal title
                                    $('#modal-produto-title').text("Alterar Tipo extra");
                                    // show udpate produto button
                                    $('#btn-update-produto').show();
                                    // show create produto button
                                    $('#btn-create-produto').hide();
                                    // post id of produto to be edited
                                    $http.post('<?= base_url("index.php/ControladorPizzas/buscarTipoExtrasPorCodigo") ?>', {
                                        'codigo_tipo_extra_pizza': id
                                    }).success(function (data, status, headers, config) {
                                        // put the values in form
                                        $scope.codigo_tipo_extra_pizza = data["codigo_tipo_extra_pizza"];
                                        $scope.descricaoInput = data["descricao_tipo_extra_pizza"];
                                        $scope.precoInput = parseInt(data["quantidade_tipo_extra_pizza"]);
                                        // show modal
                                ...
                                $scope.alterarProduto = function () {
                                    $http.post('<?= base_url("index.php/ControladorPizzas/confirmarAlterarTipoExtra") ?>', {
                                        'codigo_tipo_extra_pizza': $scope.codigo_tipo_extra_pizza,
                                        'descricao_tipo_extra_pizza': $scope.descricaoInput,
                                        'quantidade_tipo_extra_pizza': $scope.precoInput
                                    }).success(function (data, status, headers, config) {
                                        ...
                                        for (numero = 0; numero < $scope.produtos.length; numero++) {
                                            if (parseInt($scope.produtos[numero].codigo_tipo_extra_pizza) === parseInt($scope.codigo_tipo_extra_pizza)) {
                                                $scope.produtos[numero].descricao_tipo_extra_pizza = $scope.descricaoInput;
                                                $scope.produtos[numero].quantidade_tipo_extra_pizza = $scope.precoInput;
                                            }
                                        }
                                        // close modal
                                        $('#modal-produto-form').closeModal();
                                ...
                                $scope.excluirProduto = function (codigo) {
                                    if (confirm("Tem certeza que deseja excluir?")) {
                                        $http.post('<?= base_url("index.php/ControladorPizzas/confirmarRemoverTipoExtra") ?>', {
                                            'codigo_tipo_extra_pizza': codigo
                                        }).success(function (data, status, headers, config) {
                                            Materialize.toast("Dados excluídos com sucesso", 4000);
                                            for (numero = 0; numero < $scope.produtos.length; numero++) {
                                                if ($scope.produtos[numero].codigo_tipo_extra_pizza === codigo) {
